...
* Un instrumento con una envolvente ADSR genérica, para el que se aprecie con claridad cada uno de sus
  ...
  <img src="ins1.PNG" width="640" align="center">
...
void Seno::command(long cmd, long note, long vel) {
  if (cmd == 9) {		//'Key' pressed: attack begins
    ...
    index=0;
    tblinc=tbl.size()/(SamplingRate/fc);
	  A = vel / 127.;
  }
...
const vector<float> & Seno::synthesize() {
  ...
  for (unsigned int i=0; i<x.size(); ++i) {
    inc = fmod(inc + tblinc,tbl.size());
    index = floor(inc);

    x[i] = A*(tbl[floor(index)]+(tbl[floor(index)+1]-tbl[floor(index)])*(inc-floor(index)));
  }
...
- Indique, a continuación, la orden necesaria para generar la señal (suponiendo que todos los archivos
  ...
  Situandose en el directorio `work/music` se ejecuta:
  ```
  synth sinfm.orc ToyStory_A_Friend_in_me.sco ToyStory_A_Friend_in_me.wav
  ```
